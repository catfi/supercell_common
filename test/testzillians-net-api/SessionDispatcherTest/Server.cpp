...
struct move_message// : ObjectPool<move_message>
{
	enum { TYPE = 0 };
...
void handle_message_read(TcpSession& session, move_message& message)
{
	client_context* ctx = session.getContext<client_context>();
	...
	try
	{
		//LOG4CXX_INFO(gLogger, "message: id = " << message.id << ", positionX = " << message.positionX << ", positionY = " << message.positionY << ", distance = " << message.distance);
		session.writeAsync(message, boost::bind(handle_message_write, &session, placeholders::error));
...
void handle_buffer_read(TcpSession& session, uint32 type, shared_ptr<Buffer>& buffer, std::size_t size)
{
	...
	{
		//LOG4CXX_INFO(gLogger, "message: id = " << message.id << ", positionX = " << message.positionX << ", positionY = " << message.positionY << ", distance = " << message.distance);
		//session.writeAsync(type, buffer);
		session.write(type, buffer, size);
	}
	catch(std::exception& e)
	{
		LOG4CXX_INFO(gLogger, "client #" << ctx->id << " failed write, error = " << e.what());
...
#ifdef USE_MESSAGE_DISPATCH
	engine.getDispatcher().bind<move_message>(
			boost::bind(handle_message_read,
