...
log4cxx::LoggerPtr gLogger(log4cxx::Logger::getLogger("TimeSynchronizerServer"));
...
void print_current_time(const boost::system::error_code& ec, TimeSynchronizer<TcpSessionEngine, TcpSession>* synchronizer, boost::asio::deadline_timer* timer)
{
	LOG4CXX_DEBUG(gLogger, " current time = " << synchronizer->current());
	timer->expires_from_now(boost::posix_time::milliseconds(2000));
	timer->async_wait(boost::bind(print_current_time, placeholders::error, synchronizer, timer));
}
...
int main(int argc, char** argv)
{
	...
	TcpSessionEngine engine(&ios);

	TimeSynchronizer<TcpSessionEngine, TcpSession> synchronizer(engine);
