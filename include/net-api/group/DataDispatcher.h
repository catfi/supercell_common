 ...
 * a buffer completion into a message handler (in order to support functor).
 */
template <typename Source, typename M>
struct MessageDispatchT : public DataDispatchT<Source>
{
...
public:
	 ...
	 * @param maxType
	 */
	DataDispatcherT(uint32 maxType) : mMaxDispatchType(maxType)
	{
		mDispatchByType = new DataDispatchT<Source>*[mMaxDispatchType];
...
public:
	 ...
	 * @param handler
	 */
	void bind(uint32 type, DataDispatchT<Source>* handler)
	{
...
private:
	...
	DataDispatchT<Source>** mDispatchByType;

	/// The dispatch implementation for any not-registered buffer completion type
	DataDispatchT<Source>*  mDispatchAny;

	// TODO handle messages
	// TODO handle membership change info as well
};

} } }

#endif/*ZILLIANS_NET_GROUP_DATADISPATCHER_H_*/
